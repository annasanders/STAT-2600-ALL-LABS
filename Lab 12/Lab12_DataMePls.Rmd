...
__Question:__ How does the average life expectancy for the American countries compared to that of other countries around the world?
...
The plot shows a normal distribution of data when tested over 1000 random samples. The mean of the sample data is roughly centered around 0.634 which would imply that the two factors are moderately to highly correlated. One standard deviation of the data is roughly 0.018, so the correlation is pretty invariant. A blue vertical line is plotted to show where the actual correlation between the data is (without permutation tests). Since our permutation tests are done 1000 times, the central limit theorem holds on our data being approximated by a normal distribution as yields accurate data for the calculated correlation between life expectancy and GDP per capita. The median data statistic for the permutation test is 0.6336 and the 1st quadrant begins at 0.6226 while the third quadrant begins as 0.6453, so since the actual correlation lies within this data, we would reject the null hypothesis which says that the correlation of the data is zero. 

We created a function that took in both life expectancy and GDP per capita vectors. Then, we used 1000 permutations of randomizing the life expectancy vector and computed the correlation between those and the original GDP per capita vector and stored those in an empty matrix. We then plotted these computed correlations and plotted them in a histogram and then plotted the actual data correlation as a vertical line on the histogram. We then adapted these functions and plots to the new gapminder_unfiltered data with tweaking just the import part. 

# Who did what:
__Two- Sample Permutation Test:__ Amanda and James

__Corelation Permuation Test:__ Chris

__Moral Support:__ Anna (I'm tried to help but coding is not my strong suit)