...
perm_mean <- function(perms = 1000, values, n1)
{
  ...
  for (i in c(1:perms))
  {
    ...
    # Randomly separate vector "values" into disjoint 
    # groups of size "n1" and "length(values) - n1" respectively
    size <- length(values) - length(n1)
    randomSamp <- sample(values, size, replace = FALSE)
    randomSamp2 <- sample(values, length(n1), replace = FALSE)
