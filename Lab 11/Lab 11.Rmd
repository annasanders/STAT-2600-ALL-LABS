...
## Amanda
```{r, eval=FALSE}
amanda <- babynames %>% filter(str_detect(name, "Amanda"), sex=="F") %>% group_by(year)
#view(amanda)
...
simNames <- babynames %>% filter(str_detect(name, "^Ama"), sex=='F') %>% group_by(year) %>% summarise(sum(n)) %>% rename(count = 'sum(n)')
#view(simNames)
...
ggplot()+
  ...
  xlab("Year")+
  ylab("Count of Names")
  ggtitle("Number of 'Amanda' Names\nVersus 'Ama' Names")
...
ggplot(data=amanda)+
  ...
  ggtitle("Proportion of Names That Are 'Amanda'\nThroughout the Years")
```
